...
loadTest.init = function(options) {
  ...
  if (!options.numberOfRequestss) {
    console.log('options.numberOfRequests is undefined set it to : '  + loadTest.defaultOptions.numberOfRequests);
  } else {
    console.log('Going to Run: ' + options.numberOfRequestss + ' times.');
  }
...
loadTest.requestAtInterval = function () {
  ...
  .take(loadTest.defaultOptions.numberOfRequests)
  .through(function (stream) {
    return loadTest.defaultOptions.runInParallel ? stream.merge() : stream.series();
  })
  .on('end', function () {
    console.log('>>>> Finished <<<<');
    console.log('success rate of: ' + successes/loadTest.defaultOptions.numberOfRequests * 100 + '%');
    console.log('failure rate of: ' + (loadTest.defaultOptions.numberOfRequests - successes)/loadTest.defaultOptions.numberOfRequests * 100 + '%');
    process.exit(0);
  })
  .each(function (res) {
