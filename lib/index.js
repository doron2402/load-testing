...
loadTest.requestAtInterval = function (url, optsIn) {
  var opts = _.extend({
    timeInterval: 200,
    limit: 25,
    parallel: true,
  ...
  .ratelimit(1, opts.timeInterval)
  .take(opts.limit)
  .through(function (stream) {
    return opts.parallel ? stream.merge() : stream.series();
  })
  .on('end', function () {
    console.log('Finished with a success rate of: ' + successes/opts.limit * 100 + '%');
  })
  .each(function (res) {
    if (/2\d\d/.test(String(res.statusCode))) { successes++ }
